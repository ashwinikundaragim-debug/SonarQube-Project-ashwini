pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'ashwinikmath'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/ashwinikundaragim-debug/SonarQube-Project-ashwini.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Ashwini -Dsonar.projectKey=AshwiniKey -Dsonar.java.binaries=target"
                }
            }
        }
